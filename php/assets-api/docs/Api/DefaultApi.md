...
```php
getLootTablesV2LootTablesGet($client_version): array<string,\OpenAPI\Client\Model\LootTableV2>
```
...
try {
    $result = $apiInstance->getLootTablesV2LootTablesGet($client_version);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling DefaultApi->getLootTablesV2LootTablesGet: ', $e->getMessage(), PHP_EOL;
}
```
...
[**array<string,\OpenAPI\Client\Model\LootTableV2>**](../Model/LootTableV2.md)
