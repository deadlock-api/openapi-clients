...
import {
    LootTableV2Entry,
    LootTableV2EntryFromJSON,
    LootTableV2EntryToJSON,
} from './';

/**
 * 
 * @export
 * @interface LootTableV2
 */
export interface LootTableV2  {
    /**
     * 
     * @type {Array<LootTableV2Entry>}
     * @memberof LootTableV2
     */
    entries: Array<LootTableV2Entry>;
}

export function LootTableV2FromJSON(json: any): LootTableV2 {
    return {
        'entries': (json['entries'] as Array<any>).map(LootTableV2EntryFromJSON),
    };
}

export function LootTableV2ToJSON(value?: LootTableV2): any {
    if (value === undefined) {
        return undefined;
    }
    return {
        'entries': (value.entries as Array<any>).map(LootTableV2EntryToJSON),
    };
}
