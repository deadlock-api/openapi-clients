// tslint:disable
/**
 * Deadlock API
 *  ## API Clients  We have auto generated and updated clients for many languages. You can find them here: [https://github.com/deadlock-api/openapi-clients](https://github.com/deadlock-api/openapi-clients)  ## Support the Deadlock API  Whether you\'re building your own database, developing data science projects, or enhancing your website with game and player analytics, the Deadlock API has the data you need.  Your sponsorship helps keep this resource open, free and future-proof for everyone. By supporting the Deadlock API, you will enable continued development, new features and reliable access for developers, analysts and streamers worldwide.  Help us continue to provide the data you need - sponsor the Deadlock API today!  **-> You can Sponsor the Deadlock API on [Patreon](https://www.patreon.com/c/user?u=68961896) or [GitHub](https://github.com/sponsors/raimannma)**  ## Disclaimer _deadlock-api.com is not endorsed by Valve and does not reflect the views or opinions of Valve or anyone officially involved in producing or managing Valve properties. Valve and all associated properties are trademarks or registered trademarks of Valve Corporation_         
 *
 * The version of the OpenAPI document: 0.1.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import {
    JobStatus,
    JobStatusFromJSON,
    JobStatusToJSON,
    OutputFormat,
    OutputFormatFromJSON,
    OutputFormatToJSON,
} from './';

/**
 * 
 * @export
 * @interface DemoQueryStatusResponse
 */
export interface DemoQueryStatusResponse  {
    /**
     * Failure reason, once `failed`.
     * @type {string}
     * @memberof DemoQueryStatusResponse
     */
    error?: string;
    /**
     * Rough seconds until the result is ready, while `queued` or `running`.
     * @type {number}
     * @memberof DemoQueryStatusResponse
     */
    estimatedWaitSeconds?: number;
    /**
     * 
     * @type {OutputFormat}
     * @memberof DemoQueryStatusResponse
     */
    format: OutputFormat;
    /**
     * 
     * @type {string}
     * @memberof DemoQueryStatusResponse
     */
    jobId: string;
    /**
     * 
     * @type {number}
     * @memberof DemoQueryStatusResponse
     */
    matchId: number;
    /**
     * Public URL of the result artifact, once `done`.
     * @type {string}
     * @memberof DemoQueryStatusResponse
     */
    resultUrl?: string;
    /**
     * 
     * @type {JobStatus}
     * @memberof DemoQueryStatusResponse
     */
    status: JobStatus;
}

export function DemoQueryStatusResponseFromJSON(json: any): DemoQueryStatusResponse {
    return {
        'error': !exists(json, 'error') ? undefined : json['error'],
        'estimatedWaitSeconds': !exists(json, 'estimated_wait_seconds') ? undefined : json['estimated_wait_seconds'],
        'format': OutputFormatFromJSON(json['format']),
        'jobId': json['job_id'],
        'matchId': json['match_id'],
        'resultUrl': !exists(json, 'result_url') ? undefined : json['result_url'],
        'status': JobStatusFromJSON(json['status']),
    };
}

export function DemoQueryStatusResponseToJSON(value?: DemoQueryStatusResponse): any {
    if (value === undefined) {
        return undefined;
    }
    return {
        'error': value.error,
        'estimated_wait_seconds': value.estimatedWaitSeconds,
        'format': OutputFormatToJSON(value.format),
        'job_id': value.jobId,
        'match_id': value.matchId,
        'result_url': value.resultUrl,
        'status': JobStatusToJSON(value.status),
    };
}



