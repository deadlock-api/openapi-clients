...
export interface LootTable  {
    /**
     * 
     * @type {Array<LootEntry>}
     * @memberof LootTable
     */
    entries: Array<LootEntry>;
}

export function LootTableFromJSON(json: any): LootTable {
    return {
        'entries': (json['entries'] as Array<any>).map(LootEntryFromJSON),
    };
}

export function LootTableToJSON(value?: LootTable): any {
    if (value === undefined) {
        return undefined;
    }
    return {
        'entries': (value.entries as Array<any>).map(LootEntryToJSON),
    };
}
