 ...
 * Get Misc Entity
 */
function getMiscEntityRaw<T>(requestParameters: GetMiscEntityRequest, requestConfig: runtime.TypedQueryConfig<T, MiscEntity> = {}): QueryConfig<T> {
    if (requestParameters.idOrClassname === null || requestParameters.idOrClassname === undefined) {
        throw new runtime.RequiredError('idOrClassname','Required parameter requestParameters.idOrClassname was null or undefined when calling getMiscEntity.');
    }
    ...
    const config: QueryConfig<T> = {
        url: `${runtime.Configuration.basePath}/v1/assets/misc-entities/{id_or_classname}`.replace('{id_or_classname}', encodeURIComponent(String(requestParameters.idOrClassname))),
        meta,
    ...
    if (requestTransform) {
        config.transform = (body: ResponseBody, text: ResponseBody) => requestTransform(MiscEntityFromJSON(body), text);
    }
...
* Get Misc Entity
*/
export function getMiscEntity<T>(requestParameters: GetMiscEntityRequest, requestConfig?: runtime.TypedQueryConfig<T, MiscEntity>): QueryConfig<T> {
    return getMiscEntityRaw(requestParameters, requestConfig);
}
...
function listMiscEntitiesRaw<T>(requestParameters: ListMiscEntitiesRequest, requestConfig: runtime.TypedQueryConfig<T, Array<MiscEntity>> = {}): QueryConfig<T> {
    ...
    const config: QueryConfig<T> = {
        url: `${runtime.Configuration.basePath}/v1/assets/misc-entities`,
        meta,
        update: requestConfig.update,
        queryKey: requestConfig.queryKey,
        optimisticUpdate: requestConfig.optimisticUpdate,
    ...
    const { transform: requestTransform } = requestConfig;
    if (requestTransform) {
        config.transform = (body: ResponseBody, text: ResponseBody) => requestTransform(body.map(MiscEntityFromJSON), text);
    }
