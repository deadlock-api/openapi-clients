// <auto-generated>
/*
 * Deadlock API
 *
 *  ## API Clients  We have auto generated and updated clients for many languages. You can find them here: [https://github.com/deadlock-api/openapi-clients](https://github.com/deadlock-api/openapi-clients)  ## Support the Deadlock API  Whether you're building your own database, developing data science projects, or enhancing your website with game and player analytics, the Deadlock API has the data you need.  Your sponsorship helps keep this resource open, free and future-proof for everyone. By supporting the Deadlock API, you will enable continued development, new features and reliable access for developers, analysts and streamers worldwide.  Help us continue to provide the data you need - sponsor the Deadlock API today!  **-> You can Sponsor the Deadlock API on [Patreon](https://www.patreon.com/c/user?u=68961896) or [GitHub](https://github.com/sponsors/raimannma)**  ## Disclaimer _deadlock-api.com is not endorsed by Valve and does not reflect the views or opinions of Valve or anyone officially involved in producing or managing Valve properties. Valve and all associated properties are trademarks or registered trademarks of Valve Corporation_         
 *
 * The version of the OpenAPI document: 0.1.0
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */

#nullable enable

using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;
using System.Text.Json;
using System.Text.Json.Serialization;
using System.ComponentModel.DataAnnotations;
using DeadlockApiClient.Client;

namespace DeadlockApiClient.Model
{
    /// <summary>
    /// Hero
    /// </summary>
    public partial class Hero : IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="Hero" /> class.
        /// </summary>
        /// <param name="assignedPlayersOnly">assignedPlayersOnly</param>
        /// <param name="className">className</param>
        /// <param name="colors">colors</param>
        /// <param name="complexity">complexity</param>
        /// <param name="description">description</param>
        /// <param name="disabled">disabled</param>
        /// <param name="heroStatsUi">heroStatsUi</param>
        /// <param name="id">id</param>
        /// <param name="images">images</param>
        /// <param name="inDevelopment">inDevelopment</param>
        /// <param name="itemSlotInfo">itemSlotInfo</param>
        /// <param name="items">items</param>
        /// <param name="levelInfo">levelInfo</param>
        /// <param name="limitedTesting">limitedTesting</param>
        /// <param name="name">name</param>
        /// <param name="needsTesting">needsTesting</param>
        /// <param name="physics">physics</param>
        /// <param name="playerSelectable">playerSelectable</param>
        /// <param name="purchaseBonuses">purchaseBonuses</param>
        /// <param name="scalingStats">scalingStats</param>
        /// <param name="shopStatDisplay">shopStatDisplay</param>
        /// <param name="skin">skin</param>
        /// <param name="standardLevelUpUpgrades">standardLevelUpUpgrades</param>
        /// <param name="startingStats">startingStats</param>
        /// <param name="statsDisplay">statsDisplay</param>
        /// <param name="tags">Always emitted (empty if the hero declares no &#x60;m_vecHeroTags&#x60;).</param>
        /// <param name="costBonuses">costBonuses</param>
        /// <param name="gunTag">gunTag</param>
        /// <param name="heroType">heroType</param>
        /// <param name="hideoutRichPresence">hideoutRichPresence</param>
        /// <param name="itemDraftBucketing">itemDraftBucketing</param>
        /// <param name="itemDraftWeights">itemDraftWeights</param>
        /// <param name="prereleaseOnly">prereleaseOnly</param>
        [JsonConstructor]
        public Hero(bool assignedPlayersOnly, string className, HeroColors colors, long complexity, HeroDescription description, bool disabled, HeroStatsUI heroStatsUi, int id, HeroImages images, bool inDevelopment, Dictionary<string, HashMapItemSlotTypeItemSlotInfoValue> itemSlotInfo, Dictionary<string, string> items, Dictionary<string, HashMapStringLevelInfoValue> levelInfo, bool limitedTesting, string name, bool needsTesting, HeroPhysics physics, bool playerSelectable, Dictionary<string, List<HashMapItemSlotTypeVecPurchaseBonusValueInner>> purchaseBonuses, Dictionary<string, HashMapStringScalingStatValue> scalingStats, ShopStatDisplay shopStatDisplay, long skin, Dictionary<string, double> standardLevelUpUpgrades, StartingStats startingStats, StatsDisplay statsDisplay, List<string> tags, Option<Dictionary<string, List<HashMapItemSlotTypeVecMapModCostBonusValueInner>>?> costBonuses = default, Option<string?> gunTag = default, Option<HeroType?> heroType = default, Option<string?> hideoutRichPresence = default, Option<Dictionary<string, HashMapStringOptionDraftBucketingValue>?> itemDraftBucketing = default, Option<Dictionary<string, double>?> itemDraftWeights = default, Option<bool?> prereleaseOnly = default)
        {
            AssignedPlayersOnly = assignedPlayersOnly;
            ClassName = className;
            Colors = colors;
            Complexity = complexity;
            Description = description;
            Disabled = disabled;
            HeroStatsUi = heroStatsUi;
            Id = id;
            Images = images;
            InDevelopment = inDevelopment;
            ItemSlotInfo = itemSlotInfo;
            Items = items;
            LevelInfo = levelInfo;
            LimitedTesting = limitedTesting;
            Name = name;
            NeedsTesting = needsTesting;
            Physics = physics;
            PlayerSelectable = playerSelectable;
            PurchaseBonuses = purchaseBonuses;
            ScalingStats = scalingStats;
            ShopStatDisplay = shopStatDisplay;
            Skin = skin;
            StandardLevelUpUpgrades = standardLevelUpUpgrades;
            StartingStats = startingStats;
            StatsDisplay = statsDisplay;
            Tags = tags;
            CostBonusesOption = costBonuses;
            GunTagOption = gunTag;
            HeroTypeOption = heroType;
            HideoutRichPresenceOption = hideoutRichPresence;
            ItemDraftBucketingOption = itemDraftBucketing;
            ItemDraftWeightsOption = itemDraftWeights;
            PrereleaseOnlyOption = prereleaseOnly;
            OnCreated();
        }

        partial void OnCreated();

        /// <summary>
        /// Used to track the state of HeroType
        /// </summary>
        [JsonIgnore]
        [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
        public Option<HeroType?> HeroTypeOption { get; private set; }

        /// <summary>
        /// Gets or Sets HeroType
        /// </summary>
        [JsonPropertyName("hero_type")]
        public HeroType? HeroType { get { return this.HeroTypeOption.Value; } set { this.HeroTypeOption = new(value); } }

        /// <summary>
        /// Gets or Sets AssignedPlayersOnly
        /// </summary>
        [JsonPropertyName("assigned_players_only")]
        public bool AssignedPlayersOnly { get; set; }

        /// <summary>
        /// Gets or Sets ClassName
        /// </summary>
        [JsonPropertyName("class_name")]
        public string ClassName { get; set; }

        /// <summary>
        /// Gets or Sets Colors
        /// </summary>
        [JsonPropertyName("colors")]
        public HeroColors Colors { get; set; }

        /// <summary>
        /// Gets or Sets Complexity
        /// </summary>
        [JsonPropertyName("complexity")]
        public long Complexity { get; set; }

        /// <summary>
        /// Gets or Sets Description
        /// </summary>
        [JsonPropertyName("description")]
        public HeroDescription Description { get; set; }

        /// <summary>
        /// Gets or Sets Disabled
        /// </summary>
        [JsonPropertyName("disabled")]
        public bool Disabled { get; set; }

        /// <summary>
        /// Gets or Sets HeroStatsUi
        /// </summary>
        [JsonPropertyName("hero_stats_ui")]
        public HeroStatsUI HeroStatsUi { get; set; }

        /// <summary>
        /// Gets or Sets Id
        /// </summary>
        [JsonPropertyName("id")]
        public int Id { get; set; }

        /// <summary>
        /// Gets or Sets Images
        /// </summary>
        [JsonPropertyName("images")]
        public HeroImages Images { get; set; }

        /// <summary>
        /// Gets or Sets InDevelopment
        /// </summary>
        [JsonPropertyName("in_development")]
        public bool InDevelopment { get; set; }

        /// <summary>
        /// Gets or Sets ItemSlotInfo
        /// </summary>
        [JsonPropertyName("item_slot_info")]
        public Dictionary<string, HashMapItemSlotTypeItemSlotInfoValue> ItemSlotInfo { get; set; }

        /// <summary>
        /// Gets or Sets Items
        /// </summary>
        [JsonPropertyName("items")]
        public Dictionary<string, string> Items { get; set; }

        /// <summary>
        /// Gets or Sets LevelInfo
        /// </summary>
        [JsonPropertyName("level_info")]
        public Dictionary<string, HashMapStringLevelInfoValue> LevelInfo { get; set; }

        /// <summary>
        /// Gets or Sets LimitedTesting
        /// </summary>
        [JsonPropertyName("limited_testing")]
        public bool LimitedTesting { get; set; }

        /// <summary>
        /// Gets or Sets Name
        /// </summary>
        [JsonPropertyName("name")]
        public string Name { get; set; }

        /// <summary>
        /// Gets or Sets NeedsTesting
        /// </summary>
        [JsonPropertyName("needs_testing")]
        public bool NeedsTesting { get; set; }

        /// <summary>
        /// Gets or Sets Physics
        /// </summary>
        [JsonPropertyName("physics")]
        public HeroPhysics Physics { get; set; }

        /// <summary>
        /// Gets or Sets PlayerSelectable
        /// </summary>
        [JsonPropertyName("player_selectable")]
        public bool PlayerSelectable { get; set; }

        /// <summary>
        /// Gets or Sets PurchaseBonuses
        /// </summary>
        [JsonPropertyName("purchase_bonuses")]
        public Dictionary<string, List<HashMapItemSlotTypeVecPurchaseBonusValueInner>> PurchaseBonuses { get; set; }

        /// <summary>
        /// Gets or Sets ScalingStats
        /// </summary>
        [JsonPropertyName("scaling_stats")]
        public Dictionary<string, HashMapStringScalingStatValue> ScalingStats { get; set; }

        /// <summary>
        /// Gets or Sets ShopStatDisplay
        /// </summary>
        [JsonPropertyName("shop_stat_display")]
        public ShopStatDisplay ShopStatDisplay { get; set; }

        /// <summary>
        /// Gets or Sets Skin
        /// </summary>
        [JsonPropertyName("skin")]
        public long Skin { get; set; }

        /// <summary>
        /// Gets or Sets StandardLevelUpUpgrades
        /// </summary>
        [JsonPropertyName("standard_level_up_upgrades")]
        public Dictionary<string, double> StandardLevelUpUpgrades { get; set; }

        /// <summary>
        /// Gets or Sets StartingStats
        /// </summary>
        [JsonPropertyName("starting_stats")]
        public StartingStats StartingStats { get; set; }

        /// <summary>
        /// Gets or Sets StatsDisplay
        /// </summary>
        [JsonPropertyName("stats_display")]
        public StatsDisplay StatsDisplay { get; set; }

        /// <summary>
        /// Always emitted (empty if the hero declares no &#x60;m_vecHeroTags&#x60;).
        /// </summary>
        /// <value>Always emitted (empty if the hero declares no &#x60;m_vecHeroTags&#x60;).</value>
        [JsonPropertyName("tags")]
        public List<string> Tags { get; set; }

        /// <summary>
        /// Used to track the state of CostBonuses
        /// </summary>
        [JsonIgnore]
        [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
        public Option<Dictionary<string, List<HashMapItemSlotTypeVecMapModCostBonusValueInner>>?> CostBonusesOption { get; private set; }

        /// <summary>
        /// Gets or Sets CostBonuses
        /// </summary>
        [JsonPropertyName("cost_bonuses")]
        public Dictionary<string, List<HashMapItemSlotTypeVecMapModCostBonusValueInner>>? CostBonuses { get { return this.CostBonusesOption.Value; } set { this.CostBonusesOption = new(value); } }

        /// <summary>
        /// Used to track the state of GunTag
        /// </summary>
        [JsonIgnore]
        [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
        public Option<string?> GunTagOption { get; private set; }

        /// <summary>
        /// Gets or Sets GunTag
        /// </summary>
        [JsonPropertyName("gun_tag")]
        public string? GunTag { get { return this.GunTagOption.Value; } set { this.GunTagOption = new(value); } }

        /// <summary>
        /// Used to track the state of HideoutRichPresence
        /// </summary>
        [JsonIgnore]
        [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
        public Option<string?> HideoutRichPresenceOption { get; private set; }

        /// <summary>
        /// Gets or Sets HideoutRichPresence
        /// </summary>
        [JsonPropertyName("hideout_rich_presence")]
        public string? HideoutRichPresence { get { return this.HideoutRichPresenceOption.Value; } set { this.HideoutRichPresenceOption = new(value); } }

        /// <summary>
        /// Used to track the state of ItemDraftBucketing
        /// </summary>
        [JsonIgnore]
        [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
        public Option<Dictionary<string, HashMapStringOptionDraftBucketingValue>?> ItemDraftBucketingOption { get; private set; }

        /// <summary>
        /// Gets or Sets ItemDraftBucketing
        /// </summary>
        [JsonPropertyName("item_draft_bucketing")]
        public Dictionary<string, HashMapStringOptionDraftBucketingValue>? ItemDraftBucketing { get { return this.ItemDraftBucketingOption.Value; } set { this.ItemDraftBucketingOption = new(value); } }

        /// <summary>
        /// Used to track the state of ItemDraftWeights
        /// </summary>
        [JsonIgnore]
        [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
        public Option<Dictionary<string, double>?> ItemDraftWeightsOption { get; private set; }

        /// <summary>
        /// Gets or Sets ItemDraftWeights
        /// </summary>
        [JsonPropertyName("item_draft_weights")]
        public Dictionary<string, double>? ItemDraftWeights { get { return this.ItemDraftWeightsOption.Value; } set { this.ItemDraftWeightsOption = new(value); } }

        /// <summary>
        /// Used to track the state of PrereleaseOnly
        /// </summary>
        [JsonIgnore]
        [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
        public Option<bool?> PrereleaseOnlyOption { get; private set; }

        /// <summary>
        /// Gets or Sets PrereleaseOnly
        /// </summary>
        [JsonPropertyName("prerelease_only")]
        public bool? PrereleaseOnly { get { return this.PrereleaseOnlyOption.Value; } set { this.PrereleaseOnlyOption = new(value); } }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class Hero {\n");
            sb.Append("  AssignedPlayersOnly: ").Append(AssignedPlayersOnly).Append("\n");
            sb.Append("  ClassName: ").Append(ClassName).Append("\n");
            sb.Append("  Colors: ").Append(Colors).Append("\n");
            sb.Append("  Complexity: ").Append(Complexity).Append("\n");
            sb.Append("  Description: ").Append(Description).Append("\n");
            sb.Append("  Disabled: ").Append(Disabled).Append("\n");
            sb.Append("  HeroStatsUi: ").Append(HeroStatsUi).Append("\n");
            sb.Append("  Id: ").Append(Id).Append("\n");
            sb.Append("  Images: ").Append(Images).Append("\n");
            sb.Append("  InDevelopment: ").Append(InDevelopment).Append("\n");
            sb.Append("  ItemSlotInfo: ").Append(ItemSlotInfo).Append("\n");
            sb.Append("  Items: ").Append(Items).Append("\n");
            sb.Append("  LevelInfo: ").Append(LevelInfo).Append("\n");
            sb.Append("  LimitedTesting: ").Append(LimitedTesting).Append("\n");
            sb.Append("  Name: ").Append(Name).Append("\n");
            sb.Append("  NeedsTesting: ").Append(NeedsTesting).Append("\n");
            sb.Append("  Physics: ").Append(Physics).Append("\n");
            sb.Append("  PlayerSelectable: ").Append(PlayerSelectable).Append("\n");
            sb.Append("  PurchaseBonuses: ").Append(PurchaseBonuses).Append("\n");
            sb.Append("  ScalingStats: ").Append(ScalingStats).Append("\n");
            sb.Append("  ShopStatDisplay: ").Append(ShopStatDisplay).Append("\n");
            sb.Append("  Skin: ").Append(Skin).Append("\n");
            sb.Append("  StandardLevelUpUpgrades: ").Append(StandardLevelUpUpgrades).Append("\n");
            sb.Append("  StartingStats: ").Append(StartingStats).Append("\n");
            sb.Append("  StatsDisplay: ").Append(StatsDisplay).Append("\n");
            sb.Append("  Tags: ").Append(Tags).Append("\n");
            sb.Append("  CostBonuses: ").Append(CostBonuses).Append("\n");
            sb.Append("  GunTag: ").Append(GunTag).Append("\n");
            sb.Append("  HeroType: ").Append(HeroType).Append("\n");
            sb.Append("  HideoutRichPresence: ").Append(HideoutRichPresence).Append("\n");
            sb.Append("  ItemDraftBucketing: ").Append(ItemDraftBucketing).Append("\n");
            sb.Append("  ItemDraftWeights: ").Append(ItemDraftWeights).Append("\n");
            sb.Append("  PrereleaseOnly: ").Append(PrereleaseOnly).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            // Id (int) minimum
            if (this.Id < (int)0)
            {
                yield return new ValidationResult("Invalid value for Id, must be a value greater than or equal to 0.", new [] { "Id" });
            }

            yield break;
        }
    }

    /// <summary>
    /// A Json converter for type <see cref="Hero" />
    /// </summary>
    public partial class HeroJsonConverter : JsonConverter<Hero>
    {
        partial void OnCreated();

        /// <summary>
        /// Initializes a new instance of the <see cref="HeroJsonConverter" /> class.
        /// </summary>
        public HeroJsonConverter()
        {
            OnCreated();
        }

        /// <summary>
        /// Deserializes json to <see cref="Hero" />
        /// </summary>
        /// <param name="utf8JsonReader"></param>
        /// <param name="typeToConvert"></param>
        /// <param name="jsonSerializerOptions"></param>
        /// <returns></returns>
        /// <exception cref="JsonException"></exception>
        public override Hero Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            int currentDepth = utf8JsonReader.CurrentDepth;

            if (utf8JsonReader.TokenType != JsonTokenType.StartObject && utf8JsonReader.TokenType != JsonTokenType.StartArray)
                throw new JsonException();

            JsonTokenType startingTokenType = utf8JsonReader.TokenType;

            Option<bool?> assignedPlayersOnly = default;
            Option<string?> className = default;
            Option<HeroColors?> colors = default;
            Option<long?> complexity = default;
            Option<HeroDescription?> description = default;
            Option<bool?> disabled = default;
            Option<HeroStatsUI?> heroStatsUi = default;
            Option<int?> id = default;
            Option<HeroImages?> images = default;
            Option<bool?> inDevelopment = default;
            Option<Dictionary<string, HashMapItemSlotTypeItemSlotInfoValue>?> itemSlotInfo = default;
            Option<Dictionary<string, string>?> items = default;
            Option<Dictionary<string, HashMapStringLevelInfoValue>?> levelInfo = default;
            Option<bool?> limitedTesting = default;
            Option<string?> name = default;
            Option<bool?> needsTesting = default;
            Option<HeroPhysics?> physics = default;
            Option<bool?> playerSelectable = default;
            Option<Dictionary<string, List<HashMapItemSlotTypeVecPurchaseBonusValueInner>>?> purchaseBonuses = default;
            Option<Dictionary<string, HashMapStringScalingStatValue>?> scalingStats = default;
            Option<ShopStatDisplay?> shopStatDisplay = default;
            Option<long?> skin = default;
            Option<Dictionary<string, double>?> standardLevelUpUpgrades = default;
            Option<StartingStats?> startingStats = default;
            Option<StatsDisplay?> statsDisplay = default;
            Option<List<string>?> tags = default;
            Option<Dictionary<string, List<HashMapItemSlotTypeVecMapModCostBonusValueInner>>?> costBonuses = default;
            Option<string?> gunTag = default;
            Option<HeroType?> heroType = default;
            Option<string?> hideoutRichPresence = default;
            Option<Dictionary<string, HashMapStringOptionDraftBucketingValue>?> itemDraftBucketing = default;
            Option<Dictionary<string, double>?> itemDraftWeights = default;
            Option<bool?> prereleaseOnly = default;

            while (utf8JsonReader.Read())
            {
                if (startingTokenType == JsonTokenType.StartObject && utf8JsonReader.TokenType == JsonTokenType.EndObject && currentDepth == utf8JsonReader.CurrentDepth)
                    break;

                if (startingTokenType == JsonTokenType.StartArray && utf8JsonReader.TokenType == JsonTokenType.EndArray && currentDepth == utf8JsonReader.CurrentDepth)
                    break;

                if (utf8JsonReader.TokenType == JsonTokenType.PropertyName && currentDepth == utf8JsonReader.CurrentDepth - 1)
                {
                    string? localVarJsonPropertyName = utf8JsonReader.GetString();
                    utf8JsonReader.Read();

                    switch (localVarJsonPropertyName)
                    {
                        case "assigned_players_only":
                            assignedPlayersOnly = new Option<bool?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (bool?)null : utf8JsonReader.GetBoolean());
                            break;
                        case "class_name":
                            className = new Option<string?>(utf8JsonReader.GetString()!);
                            break;
                        case "colors":
                            colors = new Option<HeroColors?>(JsonSerializer.Deserialize<HeroColors>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "complexity":
                            complexity = new Option<long?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (long?)null : utf8JsonReader.GetInt64());
                            break;
                        case "description":
                            description = new Option<HeroDescription?>(JsonSerializer.Deserialize<HeroDescription>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "disabled":
                            disabled = new Option<bool?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (bool?)null : utf8JsonReader.GetBoolean());
                            break;
                        case "hero_stats_ui":
                            heroStatsUi = new Option<HeroStatsUI?>(JsonSerializer.Deserialize<HeroStatsUI>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "id":
                            id = new Option<int?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (int?)null : utf8JsonReader.GetInt32());
                            break;
                        case "images":
                            images = new Option<HeroImages?>(JsonSerializer.Deserialize<HeroImages>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "in_development":
                            inDevelopment = new Option<bool?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (bool?)null : utf8JsonReader.GetBoolean());
                            break;
                        case "item_slot_info":
                            itemSlotInfo = new Option<Dictionary<string, HashMapItemSlotTypeItemSlotInfoValue>?>(JsonSerializer.Deserialize<Dictionary<string, HashMapItemSlotTypeItemSlotInfoValue>>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "items":
                            items = new Option<Dictionary<string, string>?>(JsonSerializer.Deserialize<Dictionary<string, string>>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "level_info":
                            levelInfo = new Option<Dictionary<string, HashMapStringLevelInfoValue>?>(JsonSerializer.Deserialize<Dictionary<string, HashMapStringLevelInfoValue>>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "limited_testing":
                            limitedTesting = new Option<bool?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (bool?)null : utf8JsonReader.GetBoolean());
                            break;
                        case "name":
                            name = new Option<string?>(utf8JsonReader.GetString()!);
                            break;
                        case "needs_testing":
                            needsTesting = new Option<bool?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (bool?)null : utf8JsonReader.GetBoolean());
                            break;
                        case "physics":
                            physics = new Option<HeroPhysics?>(JsonSerializer.Deserialize<HeroPhysics>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "player_selectable":
                            playerSelectable = new Option<bool?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (bool?)null : utf8JsonReader.GetBoolean());
                            break;
                        case "purchase_bonuses":
                            purchaseBonuses = new Option<Dictionary<string, List<HashMapItemSlotTypeVecPurchaseBonusValueInner>>?>(JsonSerializer.Deserialize<Dictionary<string, List<HashMapItemSlotTypeVecPurchaseBonusValueInner>>>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "scaling_stats":
                            scalingStats = new Option<Dictionary<string, HashMapStringScalingStatValue>?>(JsonSerializer.Deserialize<Dictionary<string, HashMapStringScalingStatValue>>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "shop_stat_display":
                            shopStatDisplay = new Option<ShopStatDisplay?>(JsonSerializer.Deserialize<ShopStatDisplay>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "skin":
                            skin = new Option<long?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (long?)null : utf8JsonReader.GetInt64());
                            break;
                        case "standard_level_up_upgrades":
                            standardLevelUpUpgrades = new Option<Dictionary<string, double>?>(JsonSerializer.Deserialize<Dictionary<string, double>>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "starting_stats":
                            startingStats = new Option<StartingStats?>(JsonSerializer.Deserialize<StartingStats>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "stats_display":
                            statsDisplay = new Option<StatsDisplay?>(JsonSerializer.Deserialize<StatsDisplay>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "tags":
                            tags = new Option<List<string>?>(JsonSerializer.Deserialize<List<string>>(ref utf8JsonReader, jsonSerializerOptions)!);
                            break;
                        case "cost_bonuses":
                            costBonuses = new Option<Dictionary<string, List<HashMapItemSlotTypeVecMapModCostBonusValueInner>>?>(JsonSerializer.Deserialize<Dictionary<string, List<HashMapItemSlotTypeVecMapModCostBonusValueInner>>>(ref utf8JsonReader, jsonSerializerOptions));
                            break;
                        case "gun_tag":
                            gunTag = new Option<string?>(utf8JsonReader.GetString());
                            break;
                        case "hero_type":
                            heroType = new Option<HeroType?>(JsonSerializer.Deserialize<HeroType?>(ref utf8JsonReader, jsonSerializerOptions));
                            break;
                        case "hideout_rich_presence":
                            hideoutRichPresence = new Option<string?>(utf8JsonReader.GetString());
                            break;
                        case "item_draft_bucketing":
                            itemDraftBucketing = new Option<Dictionary<string, HashMapStringOptionDraftBucketingValue>?>(JsonSerializer.Deserialize<Dictionary<string, HashMapStringOptionDraftBucketingValue>>(ref utf8JsonReader, jsonSerializerOptions));
                            break;
                        case "item_draft_weights":
                            itemDraftWeights = new Option<Dictionary<string, double>?>(JsonSerializer.Deserialize<Dictionary<string, double>>(ref utf8JsonReader, jsonSerializerOptions));
                            break;
                        case "prerelease_only":
                            prereleaseOnly = new Option<bool?>(utf8JsonReader.TokenType == JsonTokenType.Null ? (bool?)null : utf8JsonReader.GetBoolean());
                            break;
                        default:
                            break;
                    }
                }
            }

            if (!assignedPlayersOnly.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(assignedPlayersOnly));

            if (!className.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(className));

            if (!colors.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(colors));

            if (!complexity.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(complexity));

            if (!description.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(description));

            if (!disabled.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(disabled));

            if (!heroStatsUi.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(heroStatsUi));

            if (!id.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(id));

            if (!images.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(images));

            if (!inDevelopment.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(inDevelopment));

            if (!itemSlotInfo.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(itemSlotInfo));

            if (!items.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(items));

            if (!levelInfo.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(levelInfo));

            if (!limitedTesting.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(limitedTesting));

            if (!name.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(name));

            if (!needsTesting.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(needsTesting));

            if (!physics.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(physics));

            if (!playerSelectable.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(playerSelectable));

            if (!purchaseBonuses.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(purchaseBonuses));

            if (!scalingStats.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(scalingStats));

            if (!shopStatDisplay.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(shopStatDisplay));

            if (!skin.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(skin));

            if (!standardLevelUpUpgrades.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(standardLevelUpUpgrades));

            if (!startingStats.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(startingStats));

            if (!statsDisplay.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(statsDisplay));

            if (!tags.IsSet)
                throw new ArgumentException("Property is required for class Hero.", nameof(tags));

            if (assignedPlayersOnly.IsSet && assignedPlayersOnly.Value == null)
                throw new ArgumentNullException(nameof(assignedPlayersOnly), "Property is not nullable for class Hero.");

            if (className.IsSet && className.Value == null)
                throw new ArgumentNullException(nameof(className), "Property is not nullable for class Hero.");

            if (colors.IsSet && colors.Value == null)
                throw new ArgumentNullException(nameof(colors), "Property is not nullable for class Hero.");

            if (complexity.IsSet && complexity.Value == null)
                throw new ArgumentNullException(nameof(complexity), "Property is not nullable for class Hero.");

            if (description.IsSet && description.Value == null)
                throw new ArgumentNullException(nameof(description), "Property is not nullable for class Hero.");

            if (disabled.IsSet && disabled.Value == null)
                throw new ArgumentNullException(nameof(disabled), "Property is not nullable for class Hero.");

            if (heroStatsUi.IsSet && heroStatsUi.Value == null)
                throw new ArgumentNullException(nameof(heroStatsUi), "Property is not nullable for class Hero.");

            if (id.IsSet && id.Value == null)
                throw new ArgumentNullException(nameof(id), "Property is not nullable for class Hero.");

            if (images.IsSet && images.Value == null)
                throw new ArgumentNullException(nameof(images), "Property is not nullable for class Hero.");

            if (inDevelopment.IsSet && inDevelopment.Value == null)
                throw new ArgumentNullException(nameof(inDevelopment), "Property is not nullable for class Hero.");

            if (itemSlotInfo.IsSet && itemSlotInfo.Value == null)
                throw new ArgumentNullException(nameof(itemSlotInfo), "Property is not nullable for class Hero.");

            if (items.IsSet && items.Value == null)
                throw new ArgumentNullException(nameof(items), "Property is not nullable for class Hero.");

            if (levelInfo.IsSet && levelInfo.Value == null)
                throw new ArgumentNullException(nameof(levelInfo), "Property is not nullable for class Hero.");

            if (limitedTesting.IsSet && limitedTesting.Value == null)
                throw new ArgumentNullException(nameof(limitedTesting), "Property is not nullable for class Hero.");

            if (name.IsSet && name.Value == null)
                throw new ArgumentNullException(nameof(name), "Property is not nullable for class Hero.");

            if (needsTesting.IsSet && needsTesting.Value == null)
                throw new ArgumentNullException(nameof(needsTesting), "Property is not nullable for class Hero.");

            if (physics.IsSet && physics.Value == null)
                throw new ArgumentNullException(nameof(physics), "Property is not nullable for class Hero.");

            if (playerSelectable.IsSet && playerSelectable.Value == null)
                throw new ArgumentNullException(nameof(playerSelectable), "Property is not nullable for class Hero.");

            if (purchaseBonuses.IsSet && purchaseBonuses.Value == null)
                throw new ArgumentNullException(nameof(purchaseBonuses), "Property is not nullable for class Hero.");

            if (scalingStats.IsSet && scalingStats.Value == null)
                throw new ArgumentNullException(nameof(scalingStats), "Property is not nullable for class Hero.");

            if (shopStatDisplay.IsSet && shopStatDisplay.Value == null)
                throw new ArgumentNullException(nameof(shopStatDisplay), "Property is not nullable for class Hero.");

            if (skin.IsSet && skin.Value == null)
                throw new ArgumentNullException(nameof(skin), "Property is not nullable for class Hero.");

            if (standardLevelUpUpgrades.IsSet && standardLevelUpUpgrades.Value == null)
                throw new ArgumentNullException(nameof(standardLevelUpUpgrades), "Property is not nullable for class Hero.");

            if (startingStats.IsSet && startingStats.Value == null)
                throw new ArgumentNullException(nameof(startingStats), "Property is not nullable for class Hero.");

            if (statsDisplay.IsSet && statsDisplay.Value == null)
                throw new ArgumentNullException(nameof(statsDisplay), "Property is not nullable for class Hero.");

            if (tags.IsSet && tags.Value == null)
                throw new ArgumentNullException(nameof(tags), "Property is not nullable for class Hero.");

            return new Hero(assignedPlayersOnly.Value!.Value!, className.Value!, colors.Value!, complexity.Value!.Value!, description.Value!, disabled.Value!.Value!, heroStatsUi.Value!, id.Value!.Value!, images.Value!, inDevelopment.Value!.Value!, itemSlotInfo.Value!, items.Value!, levelInfo.Value!, limitedTesting.Value!.Value!, name.Value!, needsTesting.Value!.Value!, physics.Value!, playerSelectable.Value!.Value!, purchaseBonuses.Value!, scalingStats.Value!, shopStatDisplay.Value!, skin.Value!.Value!, standardLevelUpUpgrades.Value!, startingStats.Value!, statsDisplay.Value!, tags.Value!, costBonuses, gunTag, heroType, hideoutRichPresence, itemDraftBucketing, itemDraftWeights, prereleaseOnly);
        }

        /// <summary>
        /// Serializes a <see cref="Hero" />
        /// </summary>
        /// <param name="writer"></param>
        /// <param name="hero"></param>
        /// <param name="jsonSerializerOptions"></param>
        /// <exception cref="NotImplementedException"></exception>
        public override void Write(Utf8JsonWriter writer, Hero hero, JsonSerializerOptions jsonSerializerOptions)
        {
            writer.WriteStartObject();

            WriteProperties(writer, hero, jsonSerializerOptions);
            writer.WriteEndObject();
        }

        /// <summary>
        /// Serializes the properties of <see cref="Hero" />
        /// </summary>
        /// <param name="writer"></param>
        /// <param name="hero"></param>
        /// <param name="jsonSerializerOptions"></param>
        /// <exception cref="NotImplementedException"></exception>
        public void WriteProperties(Utf8JsonWriter writer, Hero hero, JsonSerializerOptions jsonSerializerOptions)
        {
            if (hero.ClassName == null)
                throw new ArgumentNullException(nameof(hero.ClassName), "Property is required for class Hero.");

            if (hero.Colors == null)
                throw new ArgumentNullException(nameof(hero.Colors), "Property is required for class Hero.");

            if (hero.Description == null)
                throw new ArgumentNullException(nameof(hero.Description), "Property is required for class Hero.");

            if (hero.HeroStatsUi == null)
                throw new ArgumentNullException(nameof(hero.HeroStatsUi), "Property is required for class Hero.");

            if (hero.Images == null)
                throw new ArgumentNullException(nameof(hero.Images), "Property is required for class Hero.");

            if (hero.ItemSlotInfo == null)
                throw new ArgumentNullException(nameof(hero.ItemSlotInfo), "Property is required for class Hero.");

            if (hero.Items == null)
                throw new ArgumentNullException(nameof(hero.Items), "Property is required for class Hero.");

            if (hero.LevelInfo == null)
                throw new ArgumentNullException(nameof(hero.LevelInfo), "Property is required for class Hero.");

            if (hero.Name == null)
                throw new ArgumentNullException(nameof(hero.Name), "Property is required for class Hero.");

            if (hero.Physics == null)
                throw new ArgumentNullException(nameof(hero.Physics), "Property is required for class Hero.");

            if (hero.PurchaseBonuses == null)
                throw new ArgumentNullException(nameof(hero.PurchaseBonuses), "Property is required for class Hero.");

            if (hero.ScalingStats == null)
                throw new ArgumentNullException(nameof(hero.ScalingStats), "Property is required for class Hero.");

            if (hero.ShopStatDisplay == null)
                throw new ArgumentNullException(nameof(hero.ShopStatDisplay), "Property is required for class Hero.");

            if (hero.StandardLevelUpUpgrades == null)
                throw new ArgumentNullException(nameof(hero.StandardLevelUpUpgrades), "Property is required for class Hero.");

            if (hero.StartingStats == null)
                throw new ArgumentNullException(nameof(hero.StartingStats), "Property is required for class Hero.");

            if (hero.StatsDisplay == null)
                throw new ArgumentNullException(nameof(hero.StatsDisplay), "Property is required for class Hero.");

            if (hero.Tags == null)
                throw new ArgumentNullException(nameof(hero.Tags), "Property is required for class Hero.");

            writer.WriteBoolean("assigned_players_only", hero.AssignedPlayersOnly);

            writer.WriteString("class_name", hero.ClassName);

            writer.WritePropertyName("colors");
            JsonSerializer.Serialize(writer, hero.Colors, jsonSerializerOptions);
            writer.WriteNumber("complexity", hero.Complexity);

            writer.WritePropertyName("description");
            JsonSerializer.Serialize(writer, hero.Description, jsonSerializerOptions);
            writer.WriteBoolean("disabled", hero.Disabled);

            writer.WritePropertyName("hero_stats_ui");
            JsonSerializer.Serialize(writer, hero.HeroStatsUi, jsonSerializerOptions);
            writer.WriteNumber("id", hero.Id);

            writer.WritePropertyName("images");
            JsonSerializer.Serialize(writer, hero.Images, jsonSerializerOptions);
            writer.WriteBoolean("in_development", hero.InDevelopment);

            writer.WritePropertyName("item_slot_info");
            JsonSerializer.Serialize(writer, hero.ItemSlotInfo, jsonSerializerOptions);
            writer.WritePropertyName("items");
            JsonSerializer.Serialize(writer, hero.Items, jsonSerializerOptions);
            writer.WritePropertyName("level_info");
            JsonSerializer.Serialize(writer, hero.LevelInfo, jsonSerializerOptions);
            writer.WriteBoolean("limited_testing", hero.LimitedTesting);

            writer.WriteString("name", hero.Name);

            writer.WriteBoolean("needs_testing", hero.NeedsTesting);

            writer.WritePropertyName("physics");
            JsonSerializer.Serialize(writer, hero.Physics, jsonSerializerOptions);
            writer.WriteBoolean("player_selectable", hero.PlayerSelectable);

            writer.WritePropertyName("purchase_bonuses");
            JsonSerializer.Serialize(writer, hero.PurchaseBonuses, jsonSerializerOptions);
            writer.WritePropertyName("scaling_stats");
            JsonSerializer.Serialize(writer, hero.ScalingStats, jsonSerializerOptions);
            writer.WritePropertyName("shop_stat_display");
            JsonSerializer.Serialize(writer, hero.ShopStatDisplay, jsonSerializerOptions);
            writer.WriteNumber("skin", hero.Skin);

            writer.WritePropertyName("standard_level_up_upgrades");
            JsonSerializer.Serialize(writer, hero.StandardLevelUpUpgrades, jsonSerializerOptions);
            writer.WritePropertyName("starting_stats");
            JsonSerializer.Serialize(writer, hero.StartingStats, jsonSerializerOptions);
            writer.WritePropertyName("stats_display");
            JsonSerializer.Serialize(writer, hero.StatsDisplay, jsonSerializerOptions);
            writer.WritePropertyName("tags");
            JsonSerializer.Serialize(writer, hero.Tags, jsonSerializerOptions);
            if (hero.CostBonusesOption.IsSet)
                if (hero.CostBonusesOption.Value != null)
                {
                    writer.WritePropertyName("cost_bonuses");
                    JsonSerializer.Serialize(writer, hero.CostBonuses, jsonSerializerOptions);
                }
                else
                    writer.WriteNull("cost_bonuses");
            if (hero.GunTagOption.IsSet)
                if (hero.GunTagOption.Value != null)
                    writer.WriteString("gun_tag", hero.GunTag);
                else
                    writer.WriteNull("gun_tag");

            if (hero.HeroTypeOption.IsSet)
                if (hero.HeroTypeOption!.Value != null)
                {
                    var heroTypeRawValue = HeroTypeValueConverter.ToJsonValue(hero.HeroTypeOption.Value!.Value);
                    writer.WriteString("hero_type", heroTypeRawValue);
                }
                else
                    writer.WriteNull("hero_type");
            if (hero.HideoutRichPresenceOption.IsSet)
                if (hero.HideoutRichPresenceOption.Value != null)
                    writer.WriteString("hideout_rich_presence", hero.HideoutRichPresence);
                else
                    writer.WriteNull("hideout_rich_presence");

            if (hero.ItemDraftBucketingOption.IsSet)
                if (hero.ItemDraftBucketingOption.Value != null)
                {
                    writer.WritePropertyName("item_draft_bucketing");
                    JsonSerializer.Serialize(writer, hero.ItemDraftBucketing, jsonSerializerOptions);
                }
                else
                    writer.WriteNull("item_draft_bucketing");
            if (hero.ItemDraftWeightsOption.IsSet)
                if (hero.ItemDraftWeightsOption.Value != null)
                {
                    writer.WritePropertyName("item_draft_weights");
                    JsonSerializer.Serialize(writer, hero.ItemDraftWeights, jsonSerializerOptions);
                }
                else
                    writer.WriteNull("item_draft_weights");
            if (hero.PrereleaseOnlyOption.IsSet)
                if (hero.PrereleaseOnlyOption.Value != null)
                    writer.WriteBoolean("prerelease_only", hero.PrereleaseOnlyOption.Value!.Value);
                else
                    writer.WriteNull("prerelease_only");
        }
    }
}
