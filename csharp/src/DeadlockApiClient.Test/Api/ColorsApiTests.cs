...
{
    /// <summary>
    ///  Class for testing ColorsApi
    /// </summary>
    public sealed class ColorsApiTests : ApiTestsBase
    {
        private readonly IColorsApi _instance;

        public ColorsApiTests(): base(Array.Empty<string>())
        {
            _instance = _host.Services.GetRequiredService<IColorsApi>();
        }
        ...
        public async Task ListColorsAsyncTest()
        {
            Client.Option<int?> clientVersion = default!;
            var response = await _instance.ListColorsAsync(clientVersion);
            var model = response.Ok();
            Assert.IsType<Dictionary<string, Color>>(model);
        }
    }
}
