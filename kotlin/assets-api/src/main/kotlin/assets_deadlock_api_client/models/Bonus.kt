...
data class Bonus(var actualInstance: Any? = null) {

    class CustomTypeAdapterFactory : TypeAdapterFactory {
        override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T>? {
            if (!Bonus::class.java.isAssignableFrom(type.rawType)) {
            ...
            return object : TypeAdapter<Bonus?>() {
                @Throws(IOException::class)
                override fun write(out: JsonWriter,value: Bonus?) {
                    ...
                    throw IOException("Failed to serialize as the type doesn't match anyOf schemas: java.math.BigDecimal, kotlin.String")
                }

                @Throws(IOException::class)
                override fun read(jsonReader: JsonReader): Bonus {
                    ...
                    throw IOException(String.format("Failed deserialization for Bonus: no schema match result. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()))
                }
    ...
    companion object {
        ...
        * @throws IOException if the JSON Element is invalid with respect to Bonus
        */
        @Throws(IOException::class)
        fun validateJsonElement(jsonElement: JsonElement?) {
            ...
            if (match != 1) {
                throw IOException(String.format("Failed validation for Bonus: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()))
            }
        }
    }
}
