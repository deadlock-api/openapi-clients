...
data class Range(var actualInstance: Any? = null) {

    class CustomTypeAdapterFactory : TypeAdapterFactory {
        override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T>? {
            if (!Range::class.java.isAssignableFrom(type.rawType)) {
            ...
            @Suppress("UNCHECKED_CAST")
            return object : TypeAdapter<Range?>() {
                @Throws(IOException::class)
                override fun write(out: JsonWriter,value: Range?) {
                    ...
                    throw IOException("Failed to serialize as the type doesn't match anyOf schemas: java.math.BigDecimal, kotlin.collections.List<java.math.BigDecimal>")
                }

                @Throws(IOException::class)
                override fun read(jsonReader: JsonReader): Range {
                    ...
                    throw IOException(String.format("Failed deserialization for Range: no schema match result. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()))
                }
    ...
    companion object {
        ...
        */
        ...
        fun validateJsonElement(jsonElement: JsonElement?) {
            ...
            if (match != 1) {
                throw IOException(String.format("Failed validation for Range: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()))
            }
        }
    }
}
