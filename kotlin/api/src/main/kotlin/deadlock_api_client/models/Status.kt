...
data class Status (
    ...
    class CustomTypeAdapterFactory : TypeAdapterFactory {
        override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T>? {
            if (!Status::class.java.isAssignableFrom(type.rawType)) {
              return null // this class only serializes 'Status' and its subtypes
            }
            val elementAdapter = gson.getAdapter(JsonElement::class.java)
            val thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(Status::class.java))

            @Suppress("UNCHECKED_CAST")
            return object : TypeAdapter<Status>() {
        ...
        fun validateJsonElement(jsonElement: JsonElement?) {
            if (jsonElement == null) {
              require(openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
                String.format("The required field(s) %s in Status is not found in the empty JSON string", Status.openapiRequiredFields.toString())
              }
            }
            ...
            StatusServices.validateJsonElement(jsonObj["services"])
        }
    }
