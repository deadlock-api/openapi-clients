...
import type {
  PlayerStatsMetricsQueryResponse,
  PlayerStatsMetricsQueryParams,
  PlayerStatsMetrics400,
  PlayerStatsMetrics500,
} from '../types/PlayerStatsMetrics.ts'
...
export async function playerStatsMetrics(params?: PlayerStatsMetricsQueryParams, config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<PlayerStatsMetricsQueryResponse, ResponseErrorConfig<PlayerStatsMetrics400 | PlayerStatsMetrics500>, unknown>({
    method: 'GET',
    url: getPlayerStatsMetricsUrl().url.toString(),
    params,
    ...requestConfig,
  })
  return res.data
