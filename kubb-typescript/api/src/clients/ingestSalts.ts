 ...
 */
export async function ingestSalts(data?: IngestSaltsMutationRequest, config: Partial<RequestConfig<IngestSaltsMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client:request = fetch, ...requestConfig } = config

const requestData = data
const res = await request<IngestSaltsMutationResponse, ResponseErrorConfig<IngestSalts400 | IngestSalts500>, IngestSaltsMutationRequest>({ method : "POST", url : getIngestSaltsUrl().url.toString(), data : requestData, ... requestConfig })
return res.data
}