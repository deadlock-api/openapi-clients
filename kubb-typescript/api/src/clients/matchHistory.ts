...
import type { MatchHistoryQueryResponse, MatchHistoryPathParams, MatchHistoryQueryParams, MatchHistory400, MatchHistory429, MatchHistory500 } from "../types/MatchHistory.ts";
 ...
 */
export async function matchHistory(account_id: MatchHistoryPathParams["account_id"], params?: MatchHistoryQueryParams, config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  const { client:request = fetch, ...requestConfig } = config


const res = await request<MatchHistoryQueryResponse, ResponseErrorConfig<MatchHistory400 | MatchHistory429 | MatchHistory500>, unknown>({ method : "GET", url : getMatchHistoryUrl(account_id).url.toString(), params, ... requestConfig })
return res.data
}