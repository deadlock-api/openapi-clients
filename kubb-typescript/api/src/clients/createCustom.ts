 ...
 */
export async function createCustom(data?: CreateCustomMutationRequest, config: Partial<RequestConfig<CreateCustomMutationRequest>> & { client?: typeof fetch } = {}) {
  const { client:request = fetch, ...requestConfig } = config

const requestData = data
const res = await request<CreateCustomMutationResponse, ResponseErrorConfig<CreateCustom400 | CreateCustom429 | CreateCustom500>, CreateCustomMutationRequest>({ method : "POST", url : getCreateCustomUrl().url.toString(), data : requestData, ... requestConfig })
return res.data
}