...
import type {
  LeaderboardHeroRawQueryResponse,
  LeaderboardHeroRawPathParams,
  LeaderboardHeroRaw400,
  LeaderboardHeroRaw500,
} from '../types/LeaderboardHeroRaw.ts'
 ...
 */
export async function leaderboardHeroRaw(
  region: LeaderboardHeroRawPathParams['region'],
  hero_id: LeaderboardHeroRawPathParams['hero_id'],
  config: Partial<RequestConfig> & { client?: typeof fetch } = {},
) {
  const { client: request = fetch, ...requestConfig } = config

  const res = await request<LeaderboardHeroRawQueryResponse, ResponseErrorConfig<LeaderboardHeroRaw400 | LeaderboardHeroRaw500>, unknown>({
    method: 'GET',
    url: getLeaderboardHeroRawUrl(region, hero_id).url.toString(),
    ...requestConfig,
  })
  return res.data
}