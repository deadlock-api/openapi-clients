...
export async function leaderboardRaw(region: LeaderboardRawPathParams['region'], config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<LeaderboardRawQueryResponse, ResponseErrorConfig<LeaderboardRaw400 | LeaderboardRaw500>, unknown>({
    method: 'GET',
    url: getLeaderboardRawUrl(region).url.toString(),
    ...requestConfig,
  })
  return res.data
}