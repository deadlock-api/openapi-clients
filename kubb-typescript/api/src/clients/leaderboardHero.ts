 ...
 */
export async function leaderboardHero(region: LeaderboardHeroPathParams["region"], hero_id: LeaderboardHeroPathParams["hero_id"], config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  const { client:request = fetch, ...requestConfig } = config


const res = await request<LeaderboardHeroQueryResponse, ResponseErrorConfig<LeaderboardHero400 | LeaderboardHero500>, unknown>({ method : "GET", url : getLeaderboardHeroUrl(region, hero_id).url.toString(), ... requestConfig })
return res.data
}