...
export async function metadata(match_id: MetadataPathParams['match_id'], config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<MetadataQueryResponse, ResponseErrorConfig<Metadata400 | Metadata404 | Metadata429 | Metadata500>, unknown>({
    method: 'GET',
    url: getMetadataUrl(match_id).url.toString(),
    ...requestConfig,
  })
  return res.data
}