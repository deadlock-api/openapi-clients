...
/// struct for passing parameters to the method [`get_misc_entity`]
#[derive(Clone, Debug)]
...
/// struct for typed errors of method [`get_misc_entity`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetMiscEntityError {
    Status404(),
    Status500(),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`list_misc_entities`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListMiscEntitiesError {
    Status404(),
    Status500(),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_misc_entity(configuration: &configuration::Configuration, params: GetMiscEntityParams) -> Result<models::MiscEntity, Error<GetMiscEntityError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::MiscEntity`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::MiscEntity`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetMiscEntityError> = serde_json::from_str(&content).ok();
...
pub async fn list_misc_entities(configuration: &configuration::Configuration, params: ListMiscEntitiesParams) -> Result<Vec<models::MiscEntity>, Error<ListMiscEntitiesError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::MiscEntity&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::MiscEntity&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListMiscEntitiesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
