...
/// struct for passing parameters to the method [`list_loot_tables`]
#[derive(Clone, Debug)]
pub struct ListLootTablesParams {
    /// Client/game version (e.g. `6518`). Defaults to the latest known version.
    pub client_version: Option<u32>
}


/// struct for typed errors of method [`list_loot_tables`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListLootTablesError {
    Status404(),
    Status500(),
...
pub async fn list_loot_tables(configuration: &configuration::Configuration, params: ListLootTablesParams) -> Result<std::collections::HashMap<String, models::LootTable>, Error<ListLootTablesError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `std::collections::HashMap&lt;String, models::LootTable&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `std::collections::HashMap&lt;String, models::LootTable&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListLootTablesError> = serde_json::from_str(&content).ok();
