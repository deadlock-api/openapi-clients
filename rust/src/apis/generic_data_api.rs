...
/// struct for typed errors of method [`get_generic_data`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetGenericDataError {
    Status404(),
    Status500(),
...
pub async fn get_generic_data(configuration: &configuration::Configuration, params: GetGenericDataParams) -> Result<models::GenericData, Error<GetGenericDataError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GenericData`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GenericData`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetGenericDataError> = serde_json::from_str(&content).ok();
