...
pub struct SubclassObjectiveHealthGrowthPhase {
    #[serde(rename = "subclass")]
    pub subclass: Box<models::SubclassObjectiveHealthGrowthPhaseSubclass>,
}

impl SubclassObjectiveHealthGrowthPhase {
    /// Serializes back as `{\"subclass\": ...}` to preserve the KV3 wrapper shape in JSON output.
    pub fn new(subclass: models::SubclassObjectiveHealthGrowthPhaseSubclass) -> SubclassObjectiveHealthGrowthPhase {
        SubclassObjectiveHealthGrowthPhase {
            subclass: Box::new(subclass),
        }
    }
}
