...
pub async fn health_check(configuration: &configuration::Configuration) -> Result<models::Status, Error<HealthCheckError>> {
    ...
    let uri_str = format!("{}/v1/info/health", configuration.base_path);
...
pub async fn info(configuration: &configuration::Configuration) -> Result<models::ApiInfo, Error<InfoError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ApiInfo`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ApiInfo`")))),
        }
    } else {
        let content = resp.text().await?;
