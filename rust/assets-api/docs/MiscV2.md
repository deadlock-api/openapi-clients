# MiscV2

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**class_name** | **String** |  | 
**color** | Option<[**models::ColorV1**](ColorV1.md)> |  | [optional]
**initial_spawn_time** | Option<**f64**> |  | [optional]
**respawn_time** | Option<**f64**> |  | [optional]
**spawn_interval** | Option<**f64**> |  | [optional]
**initial_spawn_delay_in_seconds** | Option<**i32**> |  | [optional]
**spawn_interval_in_seconds** | Option<**i32**> |  | [optional]
**match_time_mins_for_level2_pickups** | Option<**i32**> |  | [optional]
**match_time_mins_for_level3_pickups** | Option<**i32**> |  | [optional]
**loot_list_deck_size** | Option<**i32**> |  | [optional]
**initial_spawn_delay_seconds** | Option<**i32**> |  | [optional]
**health** | Option<**i32**> |  | [optional]
**break_on_dodge_touch** | Option<**bool**> |  | [optional]
**solid_after_death** | Option<**bool**> |  | [optional]
**render_after_death** | Option<**bool**> |  | [optional]
**damaged_by_abilities** | Option<**bool**> |  | [optional]
**damaged_by_melee** | Option<**bool**> |  | [optional]
**damaged_by_bullets** | Option<**bool**> |  | [optional]
**is_mantleable** | Option<**bool**> |  | [optional]
**primary_drop_chance** | Option<**f64**> |  | [optional]
**primary_pickups** | Option<[**Vec<models::PickupDefinition>**](PickupDefinition.md)> |  | [optional]
**m_vec_pickups_lv2** | Option<[**Vec<models::PickupDefinition>**](PickupDefinition.md)> |  | [optional]
**m_vec_pickups_lv3** | Option<[**Vec<models::PickupDefinition>**](PickupDefinition.md)> |  | [optional]
**roll_type** | Option<**String**> |  | [optional]
**gold_amount** | Option<**f64**> |  | [optional]
**gold_per_minute_amount** | Option<**f64**> |  | [optional]
**modifier** | Option<[**models::SubclassModifierDefinition**](SubclassModifierDefinition.md)> |  | [optional]
**pickup_radius** | Option<[**models::PickupRadius**](Pickup_Radius.md)> |  | [optional]
**expiration_duration** | Option<[**models::ExpirationDuration**](Expiration_Duration.md)> |  | [optional]
**show_on_minimap** | Option<**bool**> |  | [optional]
**orb_spawn_delay_min** | Option<**f64**> |  | [optional]
**orb_spawn_delay_max** | Option<**f64**> |  | [optional]
**lifetime** | Option<**f64**> |  | [optional]
**collision_radius** | Option<**f64**> |  | [optional]
**id** | **i64** |  | [readonly]

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


