...
/// struct for typed errors of method [`get_misc_entities_v2_misc_entities_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetMiscEntitiesV2MiscEntitiesGetError {
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`get_misc_entity_v2_misc_entities_id_or_class_name_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetMiscEntityV2MiscEntitiesIdOrClassNameGetError {
    Status422(models::HttpValidationError),
    UnknownValue(serde_json::Value),
}


pub async fn get_misc_entities_v2_misc_entities_get(configuration: &configuration::Configuration, params: GetMiscEntitiesV2MiscEntitiesGetParams) -> Result<Vec<models::MiscV2>, Error<GetMiscEntitiesV2MiscEntitiesGetError>> {

    let uri_str = format!("{}/v2/misc-entities", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::MiscV2&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::MiscV2&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetMiscEntitiesV2MiscEntitiesGetError> = serde_json::from_str(&content).ok();
...
pub async fn get_misc_entity_v2_misc_entities_id_or_class_name_get(configuration: &configuration::Configuration, params: GetMiscEntityV2MiscEntitiesIdOrClassNameGetParams) -> Result<models::NpcUnitV2, Error<GetMiscEntityV2MiscEntitiesIdOrClassNameGetError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetMiscEntityV2MiscEntitiesIdOrClassNameGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
