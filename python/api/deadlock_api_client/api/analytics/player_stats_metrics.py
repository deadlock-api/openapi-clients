...
from ...models.hash_map import HashMap
...
) -> Optional[Union[Any, list["HashMap"]]]:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for response_200_item_data in _response_200:
            response_200_item = HashMap.from_dict(response_200_item_data)
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Any, list["HashMap"]]]:
    return Response(
...
def sync_detailed(
    *,
    ...
    > Note: Quantiles are calculated using the
    [DDSketch](https://www.vldb.org/pvldb/vol12/p2195-masson.pdf) algorithm, so they are not exact but
    have a maximum relative error of 0.01.
