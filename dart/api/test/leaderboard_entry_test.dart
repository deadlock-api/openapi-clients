//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:deadlock-api-client/api.dart';
import 'package:test/test.dart';

// tests for LeaderboardEntry
void main() {
  // final instance = LeaderboardEntry();

  group('test LeaderboardEntry', () {
    // The account name of the player.
    // String accountName
    test('to test the property `accountName`', () async {
      // TODO
    });

    // The badge level of the player. See more: <https://assets.deadlock-api.com/v2/ranks>
    // int badgeLevel
    test('to test the property `badgeLevel`', () async {
      // TODO
    });

    // The possible account IDs of the player. **CAVEAT: This is not always correct, as Steam account names are not unique.**
    // List<int> possibleAccountIds (default value: const [])
    test('to test the property `possibleAccountIds`', () async {
      // TODO
    });

    // The rank of the player. See more: <https://assets.deadlock-api.com/v2/ranks>
    // int rank
    test('to test the property `rank`', () async {
      // TODO
    });

    // The ranked rank of the player. See more: <https://assets.deadlock-api.com/v2/ranks>
    // int rankedRank
    test('to test the property `rankedRank`', () async {
      // TODO
    });

    // The ranked subrank of the player. See more: <https://assets.deadlock-api.com/v2/ranks>
    // int rankedSubrank
    test('to test the property `rankedSubrank`', () async {
      // TODO
    });

    // The top hero IDs of the player. See more: <https://assets.deadlock-api.com/v2/heroes>
    // List<int> topHeroIds (default value: const [])
    test('to test the property `topHeroIds`', () async {
      // TODO
    });


  });

}
