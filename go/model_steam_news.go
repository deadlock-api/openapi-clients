/*
Deadlock API

 ## API Clients  We have auto generated and updated clients for many languages. You can find them here: [https://github.com/deadlock-api/openapi-clients](https://github.com/deadlock-api/openapi-clients)  ## Support the Deadlock API  Whether you're building your own database, developing data science projects, or enhancing your website with game and player analytics, the Deadlock API has the data you need.  Your sponsorship helps keep this resource open, free and future-proof for everyone. By supporting the Deadlock API, you will enable continued development, new features and reliable access for developers, analysts and streamers worldwide.  Help us continue to provide the data you need - sponsor the Deadlock API today!  **-> You can Sponsor the Deadlock API on [Patreon](https://www.patreon.com/c/user?u=68961896) or [GitHub](https://github.com/sponsors/raimannma)**  ## Disclaimer _deadlock-api.com is not endorsed by Valve and does not reflect the views or opinions of Valve or anyone officially involved in producing or managing Valve properties. Valve and all associated properties are trademarks or registered trademarks of Valve Corporation_         

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package deadlock_api_client

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the SteamNews type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SteamNews{}

// SteamNews struct for SteamNews
type SteamNews struct {
	Content string `json:"content"`
	Guid PatchGuid `json:"guid"`
	Link string `json:"link"`
	PubDate time.Time `json:"pub_date"`
	Title string `json:"title"`
}

type _SteamNews SteamNews

// NewSteamNews instantiates a new SteamNews object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSteamNews(content string, guid PatchGuid, link string, pubDate time.Time, title string) *SteamNews {
	this := SteamNews{}
	this.Content = content
	this.Guid = guid
	this.Link = link
	this.PubDate = pubDate
	this.Title = title
	return &this
}

// NewSteamNewsWithDefaults instantiates a new SteamNews object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSteamNewsWithDefaults() *SteamNews {
	this := SteamNews{}
	return &this
}

// GetContent returns the Content field value
func (o *SteamNews) GetContent() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Content
}

// GetContentOk returns a tuple with the Content field value
// and a boolean to check if the value has been set.
func (o *SteamNews) GetContentOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Content, true
}

// SetContent sets field value
func (o *SteamNews) SetContent(v string) {
	o.Content = v
}

// GetGuid returns the Guid field value
func (o *SteamNews) GetGuid() PatchGuid {
	if o == nil {
		var ret PatchGuid
		return ret
	}

	return o.Guid
}

// GetGuidOk returns a tuple with the Guid field value
// and a boolean to check if the value has been set.
func (o *SteamNews) GetGuidOk() (*PatchGuid, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Guid, true
}

// SetGuid sets field value
func (o *SteamNews) SetGuid(v PatchGuid) {
	o.Guid = v
}

// GetLink returns the Link field value
func (o *SteamNews) GetLink() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Link
}

// GetLinkOk returns a tuple with the Link field value
// and a boolean to check if the value has been set.
func (o *SteamNews) GetLinkOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Link, true
}

// SetLink sets field value
func (o *SteamNews) SetLink(v string) {
	o.Link = v
}

// GetPubDate returns the PubDate field value
func (o *SteamNews) GetPubDate() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.PubDate
}

// GetPubDateOk returns a tuple with the PubDate field value
// and a boolean to check if the value has been set.
func (o *SteamNews) GetPubDateOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PubDate, true
}

// SetPubDate sets field value
func (o *SteamNews) SetPubDate(v time.Time) {
	o.PubDate = v
}

// GetTitle returns the Title field value
func (o *SteamNews) GetTitle() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Title
}

// GetTitleOk returns a tuple with the Title field value
// and a boolean to check if the value has been set.
func (o *SteamNews) GetTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Title, true
}

// SetTitle sets field value
func (o *SteamNews) SetTitle(v string) {
	o.Title = v
}

func (o SteamNews) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SteamNews) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["content"] = o.Content
	toSerialize["guid"] = o.Guid
	toSerialize["link"] = o.Link
	toSerialize["pub_date"] = o.PubDate
	toSerialize["title"] = o.Title
	return toSerialize, nil
}

func (o *SteamNews) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"content",
		"guid",
		"link",
		"pub_date",
		"title",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varSteamNews := _SteamNews{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varSteamNews)

	if err != nil {
		return err
	}

	*o = SteamNews(varSteamNews)

	return err
}

type NullableSteamNews struct {
	value *SteamNews
	isSet bool
}

func (v NullableSteamNews) Get() *SteamNews {
	return v.value
}

func (v *NullableSteamNews) Set(val *SteamNews) {
	v.value = val
	v.isSet = true
}

func (v NullableSteamNews) IsSet() bool {
	return v.isSet
}

func (v *NullableSteamNews) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSteamNews(val *SteamNews) *NullableSteamNews {
	return &NullableSteamNews{value: val, isSet: true}
}

func (v NullableSteamNews) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSteamNews) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


