/*
Deadlock API

 ## API Clients  We have auto generated and updated clients for many languages. You can find them here: [https://github.com/deadlock-api/openapi-clients](https://github.com/deadlock-api/openapi-clients)  ## Support the Deadlock API  Whether you're building your own database, developing data science projects, or enhancing your website with game and player analytics, the Deadlock API has the data you need.  Your sponsorship helps keep this resource open, free and future-proof for everyone. By supporting the Deadlock API, you will enable continued development, new features and reliable access for developers, analysts and streamers worldwide.  Help us continue to provide the data you need - sponsor the Deadlock API today!  **-> You can Sponsor the Deadlock API on [Patreon](https://www.patreon.com/c/user?u=68961896) or [GitHub](https://github.com/sponsors/raimannma)**  ## Disclaimer _deadlock-api.com is not endorsed by Valve and does not reflect the views or opinions of Valve or anyone officially involved in producing or managing Valve properties. Valve and all associated properties are trademarks or registered trademarks of Valve Corporation_         

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package deadlock_api_client

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the StatusServices type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &StatusServices{}

// StatusServices struct for StatusServices
type StatusServices struct {
	// Whether Clickhouse is reachable.
	Clickhouse bool `json:"clickhouse"`
	// Whether Postgres is reachable.
	Postgres bool `json:"postgres"`
	// Whether Redis is reachable.
	Redis bool `json:"redis"`
}

type _StatusServices StatusServices

// NewStatusServices instantiates a new StatusServices object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewStatusServices(clickhouse bool, postgres bool, redis bool) *StatusServices {
	this := StatusServices{}
	this.Clickhouse = clickhouse
	this.Postgres = postgres
	this.Redis = redis
	return &this
}

// NewStatusServicesWithDefaults instantiates a new StatusServices object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewStatusServicesWithDefaults() *StatusServices {
	this := StatusServices{}
	return &this
}

// GetClickhouse returns the Clickhouse field value
func (o *StatusServices) GetClickhouse() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.Clickhouse
}

// GetClickhouseOk returns a tuple with the Clickhouse field value
// and a boolean to check if the value has been set.
func (o *StatusServices) GetClickhouseOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Clickhouse, true
}

// SetClickhouse sets field value
func (o *StatusServices) SetClickhouse(v bool) {
	o.Clickhouse = v
}

// GetPostgres returns the Postgres field value
func (o *StatusServices) GetPostgres() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.Postgres
}

// GetPostgresOk returns a tuple with the Postgres field value
// and a boolean to check if the value has been set.
func (o *StatusServices) GetPostgresOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Postgres, true
}

// SetPostgres sets field value
func (o *StatusServices) SetPostgres(v bool) {
	o.Postgres = v
}

// GetRedis returns the Redis field value
func (o *StatusServices) GetRedis() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.Redis
}

// GetRedisOk returns a tuple with the Redis field value
// and a boolean to check if the value has been set.
func (o *StatusServices) GetRedisOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Redis, true
}

// SetRedis sets field value
func (o *StatusServices) SetRedis(v bool) {
	o.Redis = v
}

func (o StatusServices) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o StatusServices) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["clickhouse"] = o.Clickhouse
	toSerialize["postgres"] = o.Postgres
	toSerialize["redis"] = o.Redis
	return toSerialize, nil
}

func (o *StatusServices) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"clickhouse",
		"postgres",
		"redis",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varStatusServices := _StatusServices{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varStatusServices)

	if err != nil {
		return err
	}

	*o = StatusServices(varStatusServices)

	return err
}

type NullableStatusServices struct {
	value *StatusServices
	isSet bool
}

func (v NullableStatusServices) Get() *StatusServices {
	return v.value
}

func (v *NullableStatusServices) Set(val *StatusServices) {
	v.value = val
	v.isSet = true
}

func (v NullableStatusServices) IsSet() bool {
	return v.isSet
}

func (v *NullableStatusServices) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStatusServices(val *StatusServices) *NullableStatusServices {
	return &NullableStatusServices{value: val, isSet: true}
}

func (v NullableStatusServices) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStatusServices) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


