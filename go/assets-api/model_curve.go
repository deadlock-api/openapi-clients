/*
Assets - Deadlock API

 ## API Clients  We have auto generated and updated clients for many languages. You can find them here: [https://github.com/deadlock-api/openapi-clients](https://github.com/deadlock-api/openapi-clients)  ## Support the Deadlock API  Whether you're building your own database, developing data science projects, or enhancing your website with game and player analytics, the Deadlock API has the data you need.  Your sponsorship helps keep this resource open, free and future-proof for everyone. By supporting the Deadlock API, you will enable continued development, new features and reliable access for developers, analysts and streamers worldwide.  Help us continue to provide the data you need - sponsor the Deadlock API today!  **-> You can Sponsor the Deadlock API on [Patreon](https://www.patreon.com/c/user?u=68961896) or [GitHub](https://github.com/sponsors/raimannma)**  ## Disclaimer _deadlock-api.com is not endorsed by Valve and does not reflect the views or opinions of Valve or anyone officially involved in producing or managing Valve properties. Valve and all associated properties are trademarks or registered trademarks of Valve Corporation_ 

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package assets_deadlock_api_client

import (
	"encoding/json"
)

// checks if the Curve type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Curve{}

// Curve struct for Curve
type Curve struct {
	Base NullableFloat32 `json:"base,omitempty"`
	PerMinuteAfterStart NullableFloat32 `json:"per_minute_after_start,omitempty"`
}

// NewCurve instantiates a new Curve object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCurve() *Curve {
	this := Curve{}
	return &this
}

// NewCurveWithDefaults instantiates a new Curve object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCurveWithDefaults() *Curve {
	this := Curve{}
	return &this
}

// GetBase returns the Base field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Curve) GetBase() float32 {
	if o == nil || IsNil(o.Base.Get()) {
		var ret float32
		return ret
	}
	return *o.Base.Get()
}

// GetBaseOk returns a tuple with the Base field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Curve) GetBaseOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return o.Base.Get(), o.Base.IsSet()
}

// HasBase returns a boolean if a field has been set.
func (o *Curve) HasBase() bool {
	if o != nil && o.Base.IsSet() {
		return true
	}

	return false
}

// SetBase gets a reference to the given NullableFloat32 and assigns it to the Base field.
func (o *Curve) SetBase(v float32) {
	o.Base.Set(&v)
}
// SetBaseNil sets the value for Base to be an explicit nil
func (o *Curve) SetBaseNil() {
	o.Base.Set(nil)
}

// UnsetBase ensures that no value is present for Base, not even an explicit nil
func (o *Curve) UnsetBase() {
	o.Base.Unset()
}

// GetPerMinuteAfterStart returns the PerMinuteAfterStart field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Curve) GetPerMinuteAfterStart() float32 {
	if o == nil || IsNil(o.PerMinuteAfterStart.Get()) {
		var ret float32
		return ret
	}
	return *o.PerMinuteAfterStart.Get()
}

// GetPerMinuteAfterStartOk returns a tuple with the PerMinuteAfterStart field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Curve) GetPerMinuteAfterStartOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return o.PerMinuteAfterStart.Get(), o.PerMinuteAfterStart.IsSet()
}

// HasPerMinuteAfterStart returns a boolean if a field has been set.
func (o *Curve) HasPerMinuteAfterStart() bool {
	if o != nil && o.PerMinuteAfterStart.IsSet() {
		return true
	}

	return false
}

// SetPerMinuteAfterStart gets a reference to the given NullableFloat32 and assigns it to the PerMinuteAfterStart field.
func (o *Curve) SetPerMinuteAfterStart(v float32) {
	o.PerMinuteAfterStart.Set(&v)
}
// SetPerMinuteAfterStartNil sets the value for PerMinuteAfterStart to be an explicit nil
func (o *Curve) SetPerMinuteAfterStartNil() {
	o.PerMinuteAfterStart.Set(nil)
}

// UnsetPerMinuteAfterStart ensures that no value is present for PerMinuteAfterStart, not even an explicit nil
func (o *Curve) UnsetPerMinuteAfterStart() {
	o.PerMinuteAfterStart.Unset()
}

func (o Curve) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Curve) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Base.IsSet() {
		toSerialize["base"] = o.Base.Get()
	}
	if o.PerMinuteAfterStart.IsSet() {
		toSerialize["per_minute_after_start"] = o.PerMinuteAfterStart.Get()
	}
	return toSerialize, nil
}

type NullableCurve struct {
	value *Curve
	isSet bool
}

func (v NullableCurve) Get() *Curve {
	return v.value
}

func (v *NullableCurve) Set(val *Curve) {
	v.value = val
	v.isSet = true
}

func (v NullableCurve) IsSet() bool {
	return v.isSet
}

func (v *NullableCurve) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCurve(val *Curve) *NullableCurve {
	return &NullableCurve{value: val, isSet: true}
}

func (v NullableCurve) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCurve) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


