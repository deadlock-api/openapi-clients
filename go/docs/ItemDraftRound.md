# ItemDraftRound

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**NormalModTier** | **int32** |  | 
**RareModTier** | **int32** |  | 

## Methods

### NewItemDraftRound

`func NewItemDraftRound(normalModTier int32, rareModTier int32, ) *ItemDraftRound`

NewItemDraftRound instantiates a new ItemDraftRound object
This constructor will assign default values to properties that have it defined,
and makes sure properties required by API are set, but the set of arguments
will change when the set of required properties is changed

### NewItemDraftRoundWithDefaults

`func NewItemDraftRoundWithDefaults() *ItemDraftRound`

NewItemDraftRoundWithDefaults instantiates a new ItemDraftRound object
This constructor will only assign default values to properties that have it defined,
but it doesn't guarantee that properties required by API are set

### GetNormalModTier

`func (o *ItemDraftRound) GetNormalModTier() int32`

GetNormalModTier returns the NormalModTier field if non-nil, zero value otherwise.

### GetNormalModTierOk

`func (o *ItemDraftRound) GetNormalModTierOk() (*int32, bool)`

GetNormalModTierOk returns a tuple with the NormalModTier field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetNormalModTier

`func (o *ItemDraftRound) SetNormalModTier(v int32)`

SetNormalModTier sets NormalModTier field to given value.


### GetRareModTier

`func (o *ItemDraftRound) GetRareModTier() int32`

GetRareModTier returns the RareModTier field if non-nil, zero value otherwise.

### GetRareModTierOk

`func (o *ItemDraftRound) GetRareModTierOk() (*int32, bool)`

GetRareModTierOk returns a tuple with the RareModTier field if it's non-nil, zero value otherwise
and a boolean to check if the value has been set.

### SetRareModTier

`func (o *ItemDraftRound) SetRareModTier(v int32)`

SetRareModTier sets RareModTier field to given value.



[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


